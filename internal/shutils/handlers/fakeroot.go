...
func FakerootExecHandler(killTimeout time.Duration, srcDir, pkgDir string, disableNetwork bool) interp.ExecHandlerFunc {
	return func(ctx context.Context, args []string) error {
		...
		childArgs := []string{"_internal-sandbox", srcDir, pkgDir, homeDir}
		childArgs = append(childArgs, path)
		childArgs = append(childArgs, args[1:]...)

		cmd := exec.CommandContext(ctx, selfPath, childArgs...)
		...
		cmd.SysProcAttr.Cloneflags |= unix.CLONE_NEWNS | unix.CLONE_NEWPID | unix.CLONE_NEWIPC
